...
import './main.css';
import { images } from './images/ImageCollect';
...
function Main() {
    ...
    const [since, setSince] = useState(10);
            ...
            {selectedImageIndex !== null && (
                <div className="overlay" onClick={handleClose}>
                    <div className="modal" onClick={e => e.stopPropagation()}>
                        {console.log(imageData[selectedImageIndex])}

                        <button onClick={handleClose}>X</button>
                        <h2>{imageData[selectedImageIndex].name}</h2>
                        <div className="modal-details">
                            <span>{imageData[selectedImageIndex].year}</span> /
                            <span> {imageData[selectedImageIndex].genre.join(', ')}</span>
                        </div>
                        <div className="modal-content">
                            <VideoPlayer source={videoSources[selectedImageIndex]} />
                            <div className="modal-description">{imageData[selectedImageIndex].description}</div>
                        </div>
                    </div>
                </div>
            )}
